#!/bin/bash
# Push Orchestrator Workflow - FIXED V3 (Solution 5 Pattern)
# Fixes "Could not find property option" by converting options.headers to direct headers
...
FIXED_FILE="n8n-unity-build-orchestrator-FIXED-V3.json"
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}🚀 Push Orchestrator - FIXED V3 (Solution 5 Pattern)${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Fix workflow (convert options.headers to direct headers)
echo -e "${CYAN}Step 2: Fixing workflow (Solution 5 pattern - direct headers)...${NC}"
if [ ! -f "$SOURCE_FILE" ]; then
    echo -e "${RED}❌ Source file not found: $SOURCE_FILE${NC}"
    exit 1
fi

python3 scripts/fix-options-error-v3.py "$SOURCE_FILE" "$FIXED_FILE" 2>&1
...
NODE_COUNT=$(python3 -c "import json; w=json.load(open('$FIXED_FILE')); print(len(w.get('nodes', [])))" 2>/dev/null || echo "0")
if [ "$NODE_COUNT" != "13" ]; then
    echo -e "${RED}❌ ERROR: Workflow has $NODE_COUNT nodes (expected 13)${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Workflow ready (13 nodes, fixed options)${NC}"
echo ""
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "201" ]; then
    ...
    echo "   - Should load now (fixed options structure)"
    echo "   - Uses Solution 5 pattern (direct headers)"
    echo ""
    ...
    echo "   - Save each node"
    echo ""
    echo -e "${GREEN}5. Activate the workflow${NC}"
    echo ""
    ...
    echo "$BODY" | head -10
    echo ""
    echo -e "${YELLOW}💡 Try importing via UI instead:${NC}"
    echo "   1. Open n8n UI: $PI_N8N_URL"
    echo "   2. Workflows → Import from File"
    echo "   3. Select: $FIXED_FILE"
    exit 1
fi
