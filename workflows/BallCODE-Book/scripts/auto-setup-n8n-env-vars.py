...
def create_env_file():
    ...
    print("=" * 70)
    print("🤖 Automated n8n Environment Variables Setup")
    print("=" * 70)
    print()
    ...
    for key, default_value in REQUIRED_ENV_VARS.items():
        if key in existing_vars:
            ...
            print(f"✅ {key} = {existing_vars[key]} (from existing)")
        elif default_value:
            env_vars[key] = default_value
            print(f"✅ {key} = {default_value} (using default)")
        else:
            # Prompt for missing values
            value = input(f"❓ {key} (required): ").strip()
            if value:
                env_vars[key] = value
                print(f"✅ {key} = {value}")
            else:
                print(f"⚠️  {key} not set - may cause issues")
                env_vars[key] = ""
...
def create_n8n_setup_script(env_vars, env_file):
    ...
    print("📝 Creating n8n setup script...")
...
if ! curl -s http://$N8N_HOST:$N8N_PORT/healthz > /dev/null; then
    echo "❌ Cannot connect to n8n at http://$N8N_HOST:$N8N_PORT"
...
echo ""
echo "💡 Or set them in n8n's system environment:"
echo "   SSH to Pi and add to /etc/systemd/system/n8n.service"
echo "   or ~/.n8n/.env file"
echo ""
echo "✅ Setup script created: {script_file}"
"""
...
def main():
    ...
    print("=" * 70)
    print("🧪 Comprehensive n8n Test Suite")
    print("=" * 70)
    print()
    ...
    if successful == total:
        ...
        print("\\n⚠️  Some tests failed. Check n8n Executions tab for details.")
        return False
...
def main():
    ...
    print("   - Or manually: Settings → Environment Variables")
    print()
    print("2. Restart n8n:")
